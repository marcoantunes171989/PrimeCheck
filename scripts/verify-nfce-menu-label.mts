...
assert.match(validator, /\[1\/5\] Preparando ambiente/)
assert.match(validator, /\[3\/5\] Limpando build e cache do Vite/)
assert.match(validator, /\[4\/5\] Gerando e validando build/)
assert.match(validator, /\[5\/5\] Iniciando PrimeCheck em primeiro plano/)
assert.match(validator, /taskkill \/PID %%P \/F/)
assert.match(validator, /http:\/\/127\.0\.0\.1:4177/)
assert.match(validator, /npm run serve:local -- --host 127\.0\.0\.1 --port 4177 --open/)
assert.match(validator, /mantenha esta janela CMD aberta durante a validacao/)
assert.match(validator, /Ctrl\+C/)
assert.doesNotMatch(validator, /LOCAL_PORT|VITE_PID|VITE_LOG|start-local-server\.mjs|vite-local-daemon\.mjs/)
