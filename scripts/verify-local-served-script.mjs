...
const startLocalServer = readFileSync(new URL('./start-local-server.mjs', import.meta.url), 'utf8')
const viteDaemon = readFileSync(new URL('./vite-local-daemon.mjs', import.meta.url), 'utf8')
...
assert.match(validator, /start-local-server\.mjs/)
assert.match(validator, /127\.0\.0\.1:%LOCAL_PORT%/)
assert.match(validator, /PORT_READY/)
assert.match(validator, /LISTENING/)
assert.match(validator, /HOMOLOGACAO LOCAL PRONTA/)
...
assert.match(startLocalServer, /detached:\s*true/)
assert.match(startLocalServer, /windowsHide:\s*true/)
assert.match(startLocalServer, /writeFileSync\(pidFile/)
assert.match(viteDaemon, /createServer/)
assert.match(viteDaemon, /host:\s*'127\.0\.0\.1'/)
assert.match(viteDaemon, /strictPort:\s*true/)
